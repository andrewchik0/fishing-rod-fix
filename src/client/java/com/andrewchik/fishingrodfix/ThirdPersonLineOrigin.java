 ...
 * was drawn in: the collector, the {@link HandPass} frame and the count of storage clears so far. On
 * 26.1.x one {@code SubmitNodeStorage} serves every vanilla pass of a frame (the level, the
 * first-person hand, the GUI's entity pictures such as the inventory's player model), each submitted,
 * drawn and cleared in turn, so a clear ends a pass. Entities are submitted in the order the client added
 ...
 * also carries the offset the reading frame drew the body at ({@code AvatarRenderer.getRenderOffset}):
...
public final class ThirdPersonLineOrigin {
    ...
    // SubmitNodeStorage.clear calls so far, on any storage: on 26.1.x one storage serves every
    // vanilla pass of a frame, and a clear ends one. 0 while that hook doesn't apply.
    private static long clears;

    // The pass of the latest hook submitted (its collector, frame and clears), with that hook's world
     ...
     * {@code LevelRenderer.extractVisibleEntities} decides): asleep, or with the camera detached by a
    ...
    /**
     * Called where {@code ItemInHandLayer.submitArmWithItem} submits a held item, once per non-empty
     * held item of an armed entity: small, so it inlines there; only a rod in an avatar's hand in a
    ...
    public static void onItemSubmitted(ArmedEntityRenderState state, ItemStack stack, HumanoidArm arm, PoseStack poseStack, SubmitNodeCollector collector) {
        ...
        if (bodyHookFrame == HandPass.frame() && state instanceof AvatarRenderState avatar && stack != null && isRod(stack)) {
    ...
    private static void readDrawnRod(AvatarRenderState avatar, HumanoidArm arm, PoseStack poseStack, SubmitNodeCollector collector) {
        ...
        try {
            ...
            // Only while clears are counted: without, a GUI picture drawn after the level in the same
    ...
    /** Called when a {@code SubmitNodeStorage} is cleared: the pass drawn from it has ended. */
