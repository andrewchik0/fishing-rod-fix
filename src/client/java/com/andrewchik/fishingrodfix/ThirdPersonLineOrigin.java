...
public final class ThirdPersonLineOrigin {
     ...
     * entity: small, so it inlines there; only the rod arm of a player whose body-held hook was
     * extracted this frame goes on (1.21.10's call has no stack: whether the item drawn there is the
     * rod is decided out of line). Every pass extracts its entities before it submits any, so the
     * owners of the pass's hooks are known when their rods are drawn, before or after the hooks.
     */
    public static void onItemSubmitted(ArmedEntityRenderState state, Arm arm, MatrixStack matrices, OrderedRenderCommandQueue queue) {
        if (bodyHookFrame == HandPass.frame() && state instanceof PlayerEntityRenderState player
