 ...
 * paragraph). On 1.21.1 nothing is deferred and one {@code VertexConsumerProvider.Immediate} (the
 ...
 * The line's offset goes straight into {@code render}'s locals, so nothing is allocated and vanilla's
 * own {@code getHandPos} value is what every other mod still sees.
 *
 ...
 * the same in both frames, and a player's is only the crouch shift, which moves with the pose the
 * remembered path already gates on (the experimental minecart's lerp, the other part from 1.21.2
 * on, does not exist here). Re-deriving it would mean
 ...
 *
 * <p>1.21.1 extracts nothing: {@code FishingBobberEntityRenderer.render} works the origin out and
 ...
 *
 * <p>Vanilla 1.21.1 draws the world's entities in the order the client added them
 ...
 * inside the pass): that line keeps the remembered spot. (Real Camera's classic mode is the other
 * way round here - it draws the body before the entity loop reaches the hooks, so that line gets this
 * frame's rod.)
 */
public final class ThirdPersonLineOrigin {
    ...
    // renderFishingLine lifts every catenary vertex 0.25 above the pose origin, and render turns
    // getHandPos' result into the offset relative to getLerpedPos + (0, 0.25, 0).
    ...
    // Whether the hook being drawn took getHandPos' first-person branch with the camera on its owner
    // and FishingLineOrigin kept vanilla's value there (see the class Javadoc). Cleared at the start
    // of render, set inside it and read once getHandPos has returned.
    private static boolean firstPersonVanilla;

    // The decision for the hook being drawn, from the moment getHandPos returned until its line
    ...
    // into that buffer source (on 1.21.1 one of them serves every vanilla pass of a frame). The first
    ...
    // The line offset for the hook being drawn, written into render's k/l/m locals. Valid only from
     ...
     *
     * <p>Vanilla 1.21.1 does not need this: it iterates {@code ClientWorld.getEntities()} in the
     ...
     * most of them - and no allocation
     * beyond the wrapper itself - against the frustum test, the block-position lookup and the whole
     * entity render that vanilla does for each of them in the same loop. The list lives on the
    ...
    /**
     * Called once a hook's {@code getHandPos} has returned: whether its line moves onto the owner's
    ...
    public static void onHookDecided(@Nullable PlayerEntity owner, boolean onFirstPersonRod, boolean hidden, float partialTicks) {
        ...
        try {
            ...
            // The owner's partial tick, as the entity loop draws it with: players never skip a tick
            // (TickManager.shouldSkipTick), so it is the hook's unless the tick rate is frozen.
            pendingOwnerPartialTicks = player.getWorld().getTickManager().shouldTick()
                    ? partialTicks
     ...
     * fishing rod goes on. 1.21.1 hands that call
    ...
    private static void readDrawnRod(AbstractClientPlayerEntity player, Arm arm, MatrixStack matrices, VertexConsumerProvider vertexConsumers) {
        ...
        try {
            ...
            // The body as the entity loop drew it: WorldRenderer.render lerps its position with
            // tickCounter.getTickDelta(!tickManager.shouldSkipTick(entity)) and LivingEntityRenderer
            // turns it by the body yaw below.
            float partialTicks = mc.getRenderTickCounter().getTickDelta(!world.getTickManager().shouldSkipTick(player));
    ...
    public static void onHookDrawn(FishingBobberEntity hook, @Nullable PlayerEntity hookOwner, float tickDelta,
        ...
        try {
            ...
            // it is what passInverse is the inverse of. Deliberately not Entity.getLerpedPos, which
            // lerps prevX/Y/Z: render uses that one for the bobber end of the line, and the two
            // disagree whenever prev* and lastRender* do, which is part of what this fix corrects.
    ...
    public static float lineOffset(int axis, float value) {
        ...
        // m (z) is render's last offset local.
        if (axis == 2) {
            pendingOffsetValid = false;
        } else {
     ...
     * here. A player's is only the sneaking shift ({@code PlayerEntityRenderer}: {@code -2/16} times
     * the scale; the experimental minecart's lerp, the other part on later versions, does not exist
     * yet), and that moves only with the pose the remembered path already checks (a scale attribute
     * changing mid-crouch moves it too, by at most {@code 2/16} of that change). Re-deriving it here
     * would be modelling the pose, which is the one thing this class does not do.
     */
     ...
     * towards its head, within the mount's limits), mirrored. 1.21.1 keeps no render state to read it
