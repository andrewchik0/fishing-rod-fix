...
public final class FishingLineOrigin {
    ...
    /** Called with the hand FOV {@code renderWorld} got from {@code getFov}, after the entities are extracted. */
    ...
    private static @Nullable Vec3d compute(MinecraftClient mc, Camera camera, ClientPlayerEntity player) {
        ...
        // renderWorld projects the world at getFov(camera, tickProgress, true) and the hand at
        // getFov(camera, tickProgress, false), both at the window's aspect. The world FOV is this
        // frame's own. The hand FOV renderWorld asks for after the entities, so the one it took last
        // frame is used while the shared factor (see capturedSharedFactor) is the same as then: the
