 ...
 *
 * <p>Vanilla's {@code FishingBobberEntityRenderer.render} guesses the origin from the eye and the
 ...
 * {@code renderWorld}, after the world's entities are drawn) reads the live player and its
 * renderer's own state at the frame's tick progress; nothing ticks between the entity loop and the
 * hand pass, so the same reads here give the values it draws with. The bob comes from vanilla's own
 ...
 * frame, at the first drawing of the local player's hook, and reused by further drawings in it
 * (Iris' shadow pass draws the hooks first, before the world's own entity loop and its hand
 * pass).
 * <ol>
 ...
 * until the next world or dimension. The world projection itself isn't read (1.20.1 keeps no copy of
...
public final class FishingLineOrigin {
    ...
    // Inert on 1.20.1: the 2.x line starts at 1.21, and the only 1.20.1 build is Clearview 1.x
    // (mod id "clearview"), whose client mixins are the fog and the portal/spyglass overlays only,
    // so nothing on this version removes the warp and both passes get it. Kept for forward
    // compatibility.
    ...
    // lingers); the line then stays with that hand instead of vanilla's own rod-arm rule. Only
    ...
    // This frame's result (null: vanilla's value), for every later drawing of the local player's
    // hooks in the same HandPass frame: Iris' shadow pass draws the hook first, inside
    // WorldRenderer.render before the world's own entity loop, from the same frame's state (nothing
    // it reads ticks or changes in between), so the result can't differ. No reference to the player
    // or the world.
    ...
    // renderHand's only one (1.20.1 computes it there; 1.21.6 moved it up into renderWorld). Each getFov call probes the camera's surroundings for fluid
    ...
    // pair of calls each. renderWorld asks for the world FOV before the entities are drawn, so it is
    // this frame's; renderHand, which it calls at its end after them, asks for the hand FOV, so that
    // one is last frame's. On 1.20.1 renderWorld asks before its own Camera.update, so the value it
    // projects with probes last frame's camera position for fluid; the capture is that exact value,
    // while the fallback (only reached with the capture hook blocked) asks after the update and can
    // differ by the fluid factor for the one frame the eye crosses a water or lava surface. With the world FOV goes its shared
    ...
    // Doubles because 1.20.1's getFov still returns one (1.21.5 made it a float).
    private static long worldFovFrame = Long.MIN_VALUE;
    private static double capturedWorldFov;
    private static double capturedSharedFactor;
    private static long handFovFrame = Long.MIN_VALUE;
    private static double capturedHandFov;
    private static double handFovSharedFactor;
    // Relative change of the shared factor below which the hand FOV counts as unchanged: the
    // rounding of the factor's division stays far below it, while a real step (water or lava, the
    ...
    private static final double SHARED_FACTOR_TOLERANCE = 1e-5;
     ...
     * drawn, at the tick progress it asked with.
     */
    public static void onWorldFov(double fov, float tickDelta) {
        worldFovFrame = HandPass.frame();
        capturedWorldFov = fov;
        // NaN never matches: no reuse of the hand FOV unless the factor is known.
        capturedSharedFactor = Double.NaN;
        if (fovCaptureFailed) {
            return;
        }
        try {
            // GameRenderer.getFov's base with changingFov: the options FOV times the lerped movement
            // modifier, the same operations in the same order (a double times a float lerp), so the
            // factor is exactly 1 with nothing else applied.
            MinecraftClient mc = MinecraftClient.getInstance();
            GameRendererAccessor fovState = (GameRendererAccessor) mc.gameRenderer;
            double base = (double) mc.options.getFov().getValue().intValue()
     ...
     * {@code renderWorld}, after the entities are drawn.
     */
    public static void onHandFov(double fov) {
        long frame = HandPass.frame();
        handFovFrame = frame;
        capturedHandFov = fov;
        handFovSharedFactor = worldFovFrame == frame ? capturedSharedFactor : Double.NaN;
    }

    /**
     * The corrected first-person line origin as a world position, or null to keep vanilla's. Called
     * only on the first-person branch of {@code FishingBobberEntityRenderer.render}, whichever mod
     * decides that branch.
     */
    public static @Nullable Vec3d correct(PlayerEntity owner) {
        MinecraftClient mc = MinecraftClient.getInstance();
    ...
    private static @Nullable Vec3d compute(MinecraftClient mc, Camera camera, ClientPlayerEntity player) {
        ...
        // MinecraftClient.render hands GameRenderer.render, renderWorld, renderHand and so the hand
        // pass the frame's tick progress; panorama capture, the only other caller of renderWorld, is
        // excluded in correct.
        float tickDelta = FishingRodFix.tickDelta(mc);
        float swing = 0f;
        ...
        if (rodDrawn) {
            ...
            // swing); the other hand gets swing progress 0. On 1.20.1 every idle item swings the
            // same way (no swing animations yet).
            Hand attackHand = player.preferredHand != null ? player.preferredHand : Hand.MAIN_HAND;
            if (attackHand == hand) {
                swing = player.getHandSwingProgress(tickDelta);
            }
        }
        // renderItem's equip dip (1.20.1 has no per-item swap animation scale).
        ...
        double worldFov = worldFovCaptured ? capturedWorldFov : gameRenderer.fishingrodfix$getFov(camera, tickDelta, true);
        double handFov = worldFovCaptured && handFovFrame == frame - 1
        ...
        float worldPerHand = (float) (FirstPersonRod.tanHalf(worldFov) / FirstPersonRod.tanHalf(handFov));
        ...
        // rotation. 1.20.1's Camera builds its rotation as rotationYXZ(-yaw, pitch, 0), whose local
        // axes are +Z forward, +Y up and +X *left* (moveBy(forward, up, left)), so the view vector
        // goes in as (-x, y, -z). 1.21.0 changed the quaternion to rotationYXZ(PI - yaw, -pitch, 0),
        // where the same vector goes in unchanged. ---
        point.set(-point.x, point.y, -point.z).rotate(camera.getRotation());
     ...
     * {@link FishingRodFix#armHoldingRod}'s hand if a rod is drawn there, else the other one,
     * else null.
     */
    private static @Nullable Hand drawnRodHand(ClientPlayerEntity player, HeldItemRendererAccessor hands) {
        boolean holdingMain = armHoldingRod(player) == player.getMainArm();
