 ...
 * ({@code ItemInHandRenderer.submitHandsWithItems}, called from {@code renderLevel} after this frame's
 ...
 *       swing, exactly as {@code ItemInHandRenderer.submitArmWithItem} poses an idle or swinging
 *       item.</li>
...
public final class FishingLineOrigin {
    // Vanilla's item-sway factor, from ItemInHandRenderer.submitHandsWithItems().
    ...
    private static @Nullable Vec3 compute(Minecraft mc, Camera camera, LocalPlayer player) {
        ...
        if (rodDrawn) {
            ...
            // submitHandsWithItems swings only the attacking hand (swingingArm, the main hand until
