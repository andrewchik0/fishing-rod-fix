...
import com.andrewchik.fishingrodfix.mixin.client.ItemInHandRendererAccessor;
 ...
 * ({@code ItemInHandRenderer.renderHandsWithItems}, called from {@code renderLevel} after this frame's
 * extraction) reads the live player and its renderer's own state at the camera entity's partial tick;
 * nothing ticks between extraction and rendering, so the same reads here give the values it draws
 * with. The bob and both FOVs come from the camera state {@code GameRenderer.extractCamera} extracts
 * before the level's entities. The world FOV is read from the extracted projection {@code renderLevel}
 * draws the world with (in vanilla the camera's FOV), so mods that change the projection itself, not
 * only the camera's FOV, are followed too; the camera pose comes from the live {@code Camera} that
 * frame was set up from. None of it changes within the frame (the player and the renderer's state
 * only in ticks, the camera and the extracted states once per frame), so the origin is worked out
 * once per frame and reused by further extractions of the local player's hooks in it (Iris' shadow
 * pass extracts them a second time, before its hand pass).
 * <ol>
 ...
 *       swing, exactly as {@code ItemInHandRenderer.renderArmWithItem} poses an idle or swinging
 *       item.</li>
 *   <li><b>Item sway.</b> {@code renderHandsWithItems} rotates the whole hand pass by
 ...
 * local player or the camera isn't on it; when the world projection isn't a perspective one (an
 * orthographic camera mod); if the result is degenerate (a FOV from another mod out of range); and
 * after an exception, until the next world or dimension.
 * Whether the line is drawn at all is {@link FishingLineVisibility}'s call.
 */
public final class FishingLineOrigin {
    // Vanilla's item-sway factor, from ItemInHandRenderer.renderHandsWithItems().
    ...
    // a = spinningEffectTime + worldPartialTicks * spinningEffectSpeed (degrees) about the fixed
    ...
    // hooks in the same HandPass frame: Iris' shadow pass extracts the hook a second time, inside
    // renderLevel before its hand pass, from the same frame's state (nothing it reads ticks or is
    // re-extracted in between), so the result can't differ. No reference to the player or the world.
    ...
    private static @Nullable Vec3 compute(Minecraft mc, Camera camera, LocalPlayer player) {
        GameRenderState game = mc.gameRenderer.getGameRenderState();
        ...
        // Panorama capture is checked directly, on the camera as HandPass and Iris' hand renderer do:
        // its first face still sees the previous normal frame's hand pass.
        ...
        // --- 1. The rod as renderHandsWithItems / renderArmWithItem pose it this frame ---
        ...
        ItemStack drawnItem = mainHand ? hands.fishingrodfix$getMainHandItem() : hands.fishingrodfix$getOffHandItem();
        // renderLevel hands the hand pass Camera.getCameraEntityPartialTicks of the frame's delta
        // tracker: Minecraft's own, except in a panorama capture (excluded above).
        float partialTicks = camera.getCameraEntityPartialTicks(mc.getDeltaTracker());
        float swing = 0f;
        ...
        if (rodDrawn) {
            // renderHandsWithItems / renderArmWithItem's other branches: the hand not drawn, an item in
            // use in that hand (a vanilla rod has no use duration, but a modded rod might), riptide.
            boolean usingItem = player.isUsingItem() && player.getUseItemRemainingTicks() > 0 && player.getUsedItemHand() == hand;
            if (!handRendered(player, hand) || usingItem || player.isAutoSpinAttack()) {
                return null;
            }
            // renderHandsWithItems swings only the attacking hand (swingingArm, the main hand until
            // the first swing), by the drawn item's swing animation; the other hand gets attack 0.
            InteractionHand attackHand = player.swingingArm != null ? player.swingingArm : InteractionHand.MAIN_HAND;
            if (attackHand == hand) {
                float attack = player.getAttackAnim(partialTicks);
                switch (drawnItem.getSwingAnimation().type()) {
                    case WHACK -> swing = attack;
                    // SpearAnimations.firstPersonAttack leaves the rod in place only at attack 0.
                    case STAB -> {
                        if (attack > 0f) {
                            return null;
                        }
        ...
        // The renderer's own resolver is Minecraft's instance (EntityRenderDispatcher hands it over).
        float swapScale = mc.getItemModelResolver().swapAnimationScale(drawnItem);
        float inverseArmHeight = swapScale * (1f - (mainHand
                ? Mth.lerp(partialTicks, hands.fishingrodfix$getOldMainHandHeight(), hands.fishingrodfix$getMainHandHeight())
                : Mth.lerp(partialTicks, hands.fishingrodfix$getOldOffHandHeight(), hands.fishingrodfix$getOffHandHeight())));
        ...
        anchor.rotateY(Mth.DEG_TO_RAD * (player.getViewYRot(partialTicks) - yBob) * ITEM_SWAY_SCALE)
              .rotateX(Mth.DEG_TO_RAD * (player.getViewXRot(partialTicks) - xBob) * ITEM_SWAY_SCALE);
        ...
        applyWorldWarp(new Matrix4f(bob), mc, game, player).invert().transformPosition(point);
        ...
        // the origin NaN; a FOV ratio that isn't positive and finite (zero or NaN FOV or projection, a
        // lone FOV of 180-360deg) is already rejected above.
        if (!point.isFinite()) {
            return null;
        }
     ...
     */
    private static @Nullable InteractionHand drawnRodHand(LocalPlayer player, ItemInHandRendererAccessor hands) {
        boolean holdingMain = FishingHookRenderer.getHoldingArm(player) == player.getMainArm();
        ItemStack holding = holdingMain ? hands.fishingrodfix$getMainHandItem() : hands.fishingrodfix$getOffHandItem();
        ItemStack other = holdingMain ? hands.fishingrodfix$getOffHandItem() : hands.fishingrodfix$getMainHandItem();
    ...
    /**
     * Whether the hand pass draws {@code hand}: ItemInHandRenderer.evaluateWhichHandsToRender, mirrored
     * (it is package-private and returns a package-private enum, which neither an invoker nor a shadow
     * can name). Only a bow or crossbow in either hand leaves one hand out.
     */
    private static boolean handRendered(LocalPlayer player, InteractionHand hand) {
        ...
        if (!player.isUsingItem()) {
            return hand == InteractionHand.MAIN_HAND || !isChargedCrossbow(mainHandItem);
        }
        ItemStack usedItemStack = player.getUseItem();
        InteractionHand usedHand = player.getUsedItemHand();
        // Vanilla: using something else, RENDER_MAIN_HAND_ONLY with it in the main hand and a charged
        // crossbow in the off hand, else both; drawing a bow or crossbow, only that hand.
        if (!usedItemStack.is(Items.BOW) && !usedItemStack.is(Items.CROSSBOW)) {
            return hand == InteractionHand.MAIN_HAND || !(usedHand == InteractionHand.MAIN_HAND && isChargedCrossbow(offhandItem));
        }
    ...
    private static void warnIfHandPassUntracked() {
        ...
        if (++framesWithoutHandPass >= NO_HAND_PASS_WARNING_FRAMES) {
            handPassWarningLogged = true;
            LOGGER.warn("No first-person hand pass seen while fishing (its ItemInHandRenderer hook didn't apply, "
    ...
    private static Matrix4f applyWorldWarp(Matrix4f matrix, Minecraft mc, GameRenderState game, LocalPlayer player) {
        // renderLevel's own partial tick for it: the frame's world partial tick (see compute).
        float worldPartialTicks = mc.getDeltaTracker().getGameTimeDeltaPartialTick(false);
        ...
        float intensity = Math.max(portalIntensity, nauseaIntensity) * (screenEffectScale * screenEffectScale);
        if (intensity > 0f && !CLEARVIEWS_LOADED) {
            ...
            float angle = (spin.fishingrodfix$getSpinningEffectTime() + worldPartialTicks * spin.fishingrodfix$getSpinningEffectSpeed()) * Mth.DEG_TO_RAD;
