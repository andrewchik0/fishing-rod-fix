 ...
 * until the next world or dimension. The world projection itself isn't read (1.21.1 keeps no copy of
...
public final class FishingLineOrigin {
    ...
    // renderHand's only one (1.21.1 computes it there; 1.21.6 moved it up into renderWorld). Each getFov call probes the camera's surroundings for fluid
    ...
    // one is last frame's. With the world FOV goes its shared
    ...
    // Doubles because 1.21.1's getFov still returns one (1.21.5 made it a float).
    ...
    private static @Nullable Vec3d compute(MinecraftClient mc, Camera camera, ClientPlayerEntity player) {
        ...
        if (rodDrawn) {
            ...
            // swing); the other hand gets swing progress 0. On 1.21.1 every idle item swings the
        ...
        // renderItem's equip dip (1.21.1 has no per-item swap animation scale).
