 ...
 * {@code FishingBobberEntityRenderer.render}'s first-person branch anchors the line at screen point
 * {@code (±0.525, -0.1)}, and we take the point on the resting rod's mid-plane that the hand pass
 * draws there. That calibration is for vanilla's
...
final class FirstPersonRod {
    // FishingBobberEntityRenderer.render's first-person branch: camera.getProjection().getPosition(
    // i * 0.525F, -0.1F), i.e. the screen point
    ...
    // HeldItemRenderer.renderFirstPersonItem's idle branch and applySwingOffset: the swing's
    // translation scales (-0.4, 0.2, -0.2), the 45deg pre-swing turn about Y and the swing rotations
    // (-80 about X, -20 about Y and Z).
    ...
    /**
     * The right-arm pose stack before the item transform: HeldItemRenderer.renderFirstPersonItem's
     * idle branch, which on 1.20.1 translates by the swing inline, then calls applyEquipOffset and
     * applySwingOffset; the two translations commute, so the equip offset goes first here.
     */
    ...
    private static Vector2f anchorOnRestingRod() {
        float tanHalfFov = (float) tanHalf(BASE_HAND_FOV);
    ...
    /**
     * {@code tan(fov / 2)}: the view-space tangent at the screen edge for a vertical FOV. Takes a
     * double because 1.20.1's {@code GameRenderer.getFov} still returns one (1.21.5 made it a float).
     */
    static double tanHalf(double fovDegrees) {
        return Math.tan(MathHelper.RADIANS_PER_DEGREE * fovDegrees / 2.0);
    }
}
