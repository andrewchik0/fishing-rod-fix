 ...
 * a tick plus the ping (1.20.1 has no {@code /tick freeze}, which is what makes that indefinite on
 * 1.20.3+). Vanilla keeps drawing its line meanwhile,
 ...
 *
 * <p>1.20.1 has no render state: a hook's origin is worked out and its catenary drawn in one
...
public final class FishingLineVisibility {
    ...
    // once that branch has run. Any other line (on a rod held by the body, which shows the live
    ...
    /** Called from {@code render}'s first-person branch: whether the origin is on the drawn first-person rod. */
    public static void onFirstPersonOrigin(boolean onDrawnRod) {
        lineOnDrawnRod = onDrawnRod;
    }

    /** Whether the hook being drawn has its line on the drawn first-person rod (valid once the origin branch has run). */
    ...
    /** Decides, once the origin branch has run, whether the hook's line is hidden. */
