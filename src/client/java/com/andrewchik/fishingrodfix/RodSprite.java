 ...
 * tip is measured from the block-atlas sprite (1.21.1 keeps item textures there:
...
final class RodSprite {
    ...
    // (core/rendertype_entity_translucent_cull.fsh, read from the 1.21.1 client jar) discards alpha
    ...
    private static @Nullable PixelMask visiblePixels(SpriteContents sprite) {
        ...
        // animation vanilla draws tile 0 and getDistinctFrameCount() is a dummy [1]. 1.21.1's
        ...
        return (x, y) -> {
            for (int frame : visibleFrames) {
                // 1.21.1's NativeImage.getColor is public and returns ABGR (1.21.5 replaced it with
