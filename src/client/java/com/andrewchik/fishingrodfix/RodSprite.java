 ...
 * tip is measured from the block-atlas sprite (1.20.1 keeps item textures there:
...
final class RodSprite {
    private static final Identifier CAST_ROD_SPRITE = new Identifier("item/fishing_rod_cast");
    ...
    // A held rod (block atlas) draws with the entity_translucent_cull render layer: RenderLayers
    // .getItemLayer picks it over item_entity_translucent_cull for a direct-rendered, non-block item,
    // which is what both the hand pass and the held-item layer ask for. Its fragment shader
    // (core/rendertype_entity_translucent_cull.fsh, read from the 1.20.1 client jar) discards alpha
    // below 0.1, as the item-entity one does: fainter pixels are not part of the visible rod.
    ...
    // with (JsonUnbakedModel, RenderPhase).
    ...
    private static @Nullable PixelMask visiblePixels(SpriteContents sprite) {
        ...
        // animation vanilla draws tile 0 and getDistinctFrameCount() is a dummy [1]. 1.20.1's
        ...
        return (x, y) -> {
            for (int frame : visibleFrames) {
                // 1.20.1's NativeImage.getColor is public and returns ABGR (1.21.5 replaced it with
                // the ARGB getColorArgb); alpha is the top byte either way, which is all this reads.
                int pixel = image.getColor((frame % framesPerRow) * width + x, (frame / framesPerRow) * height + y);
                if (ColorHelper.Abgr.getAlpha(pixel) >= minAlpha) {
                    return true;
                }
            }
            return false;
