 ...
 * the pass).
 *
 * <p>Frames are counted at {@code GameRenderer.render}'s only {@code MinecraftClient.isFinishedLoading}
 * call: once per rendered frame, before the world (and so before {@code renderWorld}'s camera update),
 * and past {@code render}'s {@code skipGameRender}
 ...
 * entities are drawn, before this frame's hand pass, so it reads the previous frame's result: one
 * frame late when the hand appears or disappears (a mod hiding the hand, waking up). The origin is
 * worked out once per frame, at the first drawing of the local player's hook in the frame; later
 * drawings reuse that result.
 *
...
public final class HandPass {
    ...
    // first-person camera sits exactly there on 1.21.1 (Camera.update is an unconditional lerp of the
    ...
    /** Called at {@code GameRenderer.render}'s only {@code isFinishedLoading} call, once per rendered frame. */
