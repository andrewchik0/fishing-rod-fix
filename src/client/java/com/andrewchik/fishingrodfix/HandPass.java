 ...
 * the pass).
 *
 * <p>Frames are counted at {@code GameRenderer.render}'s only {@code Mouse.getX} call: once per
 * rendered frame, before the world (and so before {@code renderWorld}'s camera update), and past
 * {@code render}'s {@code skipGameRender} check, through which Dynamic FPS skips most frames while it
 * throttles (a skipped frame counted as one without a hand would put the line at vanilla's value); the
 * hand pass marks the frame when it draws a hand ({@code HeldItemRenderer.renderFirstPersonItem},
 * reached from vanilla and from Iris's hand renderer). That call is also reached while scoping, where
 * it draws nothing, so {@link FishingLineOrigin} checks scoping itself. The line origin is computed
 * while the world's entities are drawn, before this frame's hand pass, so it reads the previous
 * frame's result: one frame late when the hand appears or disappears (a mod hiding the hand, waking
 * up). The origin is worked out once per frame, at the first drawing of the local player's hook in the
 * frame; later drawings reuse that result.
 *
...
public final class HandPass {
    ...
    // first-person camera sits exactly there on 1.20.1 (Camera.update is an unconditional lerp of the
    // entity position plus the smoothed eye height); the slack is for mods that nudge the camera, and
    // for the later versions where vanilla itself moves it (an experimental-movement minecart).
    ...
    /** Called at {@code GameRenderer.render}'s only {@code Mouse.getX} call, once per rendered frame. */
    ...
    private static boolean sampleHandGate(MinecraftClient mc) {
        ...
        // Where Camera.update puts a first-person camera, at the frame's tick progress it was updated
        // with (1.20.1's Camera keeps none of its own).
