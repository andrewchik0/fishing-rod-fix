 ...
 * ({@code ItemInHandRenderer.renderArmWithItem}, reached from vanilla and from Iris's
 ...
 * the HUD and the rest of {@code renderItemInHand}'s gate (first person, not panoramic, awake, not
 * spectating; it draws the local player), plus the camera on that player at its eye, not detached
 * (freecam mods may keep the player as camera entity without detaching the camera). Whether a hand is
 * drawn when only the HUD decides is latched from frames that prove it: a drawn hand, or a missing one
 * while the HUD and that gate were open (then a mod hid it). A frame with the gate closed proves
 * nothing and restores the default (drawn), so F5, sleep or a freecam before F1 leave no stale answer.
 * So a hidden HUD doesn't move the line: F1 keeps what the line did before, and the gate still applies
 * live. What changes unseen while the HUD is hidden (a mod starting or stopping to hide the hand) only
 * shows once the HUD is back. Render-thread only.
 */
public final class HandPass {
    ...
    private static void sample(DeltaTracker deltaTracker) {
        ...
        try {
            ...
            // Options.hideGui (F1), which extractOptions copies for renderItemInHand's gate.
            hudHidden = mc.options.hideGui;
