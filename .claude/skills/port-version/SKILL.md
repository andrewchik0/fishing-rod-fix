...
lists, and the menu reached. On a bundling branch, read *which* MixinExtras won rather
than merely that one is present: the tree under `fishingrodfix` in that list, and the
`Initializing MixinExtras via … MixinExtrasServiceImpl(version=…)` line, must name the
pinned version wherever the loader's own copy is older. A `Compatibility level JAVA_xx
