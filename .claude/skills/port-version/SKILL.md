...
11. **Hand-pass hook sites** — mark the frame at the call that submits one hand
    ...
    - count only frames that render: Dynamic FPS skips rendering inside the render
      ...
      *first* call in that block there, since the profiler is reached through
      `client.getProfiler()` later — so no `ordinal` belongs on it on any of them) and check where
...
| Concern | 1.20.4 (legacy) | 1.21–1.21.1 | 1.21.4 | 1.21.5 | 1.21.6–1.21.8 | 1.21.9–1.21.10 | 1.21.11 | 26.x |
|---|---| --- |---|---|---|---|---|---|
...
| Owner's hook field (remembered rod hand) | yarn `PlayerEntity.fishHook` | same (`fishHook`) | same (`fishHook`) | same (`fishHook`) | same (`fishHook`) | same (`fishHook`) | same (set on the client by `FishingBobberEntity.setOwner`) | `Player.fishing` |
...
| Line emit path | direct vertex | direct vertex (`RenderLayer.getLineStrip()`, 17 `renderFishingLine` calls in `render`) | direct vertex (`RenderLayer.getLineStrip()`, 17 `renderFishingLine` calls in `render`) | direct vertex (`RenderLayer.getLineStrip()`, 17 `renderFishingLine` calls in `render`) | direct vertex (`RenderLayer.getLineStrip()`, 17 `renderFishingLine` calls in `render`) | deferred `submit` (the command queue exists from 1.21.9) | deferred `submit` | deferred `submit` |
